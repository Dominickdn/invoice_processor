...
def extract_invoice_data(text):
    try:
        ...
        invoice_date_str = re.search(r"Date:\s*(\d{4}-\d{2}-\d{2})", text).group(1)
        invoice_date = datetime.strptime(invoice_date_str, "%Y-%m-%d").date()
        vendor = re.search(r"Vendor:\s*(.+)", text).group(1).strip()

        items_section = re.search(r"Item Qty Unit Price Total\n(.+?)Total Amount Due:", text, re.DOTALL)
        if not items_section:
            raise ValueError("Could not find items section.")

        item_lines = [line.strip() for line in items_section.group(1).strip().split("\n") if line.strip()]
        items = []
